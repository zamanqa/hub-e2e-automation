...
// ── Prune reports to keep only the latest MAX_REPORTS ─────────────────────────
...
function pruneReports() {
  try {
    const files = fs.readdirSync(HISTORY_DIR)
      ...
      .sort((a, b) => b.localeCompare(a)); // newest first (ISO timestamp prefix)
    ...
    toDelete.forEach(f => {
      try {
        ...
        console.log(`[${ts()}] 🗑️  Pruned old report: ${f}`);
      } catch (_) {}
    });
    console.log(`[${ts()}] Pruned ${toDelete.length} old report(s), keeping latest ${MAX_REPORTS}`);
  } catch (e) {
    console.error('Could not prune reports:', e.message);
  }
}
...
function handleRun(req, res) {
  const { spec, mode } = url.parse(req.url, true).query;
  ...
  const runMode = mode === 'headless' ? 'headless' : 'headed';
  console.log(`[${ts()}] ▶  Running spec: ${spec} (${runMode})`);
  send({ status: 'starting', spec, mode: runMode });

  // Use default reporter (cypress-mochawesome-reporter from config)
  const args = ['cypress', 'run', '--browser', 'chrome'];
  if (runMode === 'headed') args.push('--headed');
  args.push('--spec', spec);
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/' || pathname === '/index.html') {
    try {
      const html = fs.readFileSync(path.join(__dirname, 'TEST_CASES.html'), 'utf8');
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      res.end(html);
    } catch (e) {
      res.writeHead(500);
      res.end('Could not read TEST_CASES.html');
    }
    return;
  }
